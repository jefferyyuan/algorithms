...
public class DFAStringSearch<E> {
	...
	public DFAStringSearch(E[] pattern) {
		...
		for (int i = 0; i < states.size(); i++) {
			...
			for (int j = s.positions.nextSetBit(0); j >= 0; j = s.positions
					.nextSetBit(j + 1)) {
				if (j == pattern.length) {
					s.finale = true;
					break;
	...
	private void fillTransitionTableEntry(E[] pattern, State<E> s, E cNext) {
		...
		newPositions.set(0);
		for (int i = s.positions.nextSetBit(0); i >= 0 && i < pattern.length; i = s.positions
				.nextSetBit(i + 1)) {
			if (pattern[i].equals(cNext))
				newPositions.set(i + 1);
		}
		s.transitions.put(cNext, getState(newPositions));
		System.err.println("Adding edge " + s + " -" + cNext + "-> "
				+ s.transitions.get(cNext));
	}
	...
	public static void main(String[] args) {
		String pattern = "ACACAGA";// "abacab";
		String text = "xabxabacaby";
		Character[] a = str2charArray(pattern), b = str2charArray(text);
		...
		if (result == -1)
			...
			System.out.println(text.substring(0, result) + "|"
